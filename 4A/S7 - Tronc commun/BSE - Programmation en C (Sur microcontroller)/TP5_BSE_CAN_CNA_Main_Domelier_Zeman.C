...
// Boucle infinie	
         ...
         {
//					 
//					 Software_Delay(1000); // attente
//					 
//					 // Test sotie AIN7 et putty
//					 AD0BUSY = 1;
//					 while(!AD0BUSY)
//						
//					 AD0INT = 0;
//					 Value_mV = (int) ((((float) ADC0/4096) * 2440)/2);
//					 CP_Mesure++;
//					 printf (" Mesure %5u -ADC0H:%#2.2bx - ADCL:%#2.2bx - Vin en mV: %#2.2u \n\r", CP_Mesure, ADC0H, ADC0L, Value_mV);
//					 
					 // Test DAC0
...
void  Voltage_Reference_Init(void)
{
	...
	REF0CN |= (1<<0); 
	REF0CN |= (1<<1); // config sortie Vref
	
	AMX0CF = 0x05;
	AMX0SL = 0x07; // config AIN0.7
	
	ADC0CF = 0x41; // clock et gain 
