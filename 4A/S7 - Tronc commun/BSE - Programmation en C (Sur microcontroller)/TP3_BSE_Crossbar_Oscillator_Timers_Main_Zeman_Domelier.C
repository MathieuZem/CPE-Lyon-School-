...
void Config_Timer2_TimeBase(void)
{
  ...
  CT2 = 0;    // Mode Timer
	RCAP2 = -((SYSCLK/12)/100*11.1); // modifié
  T2 = RCAP2;
...
void Config_Timer4_TimeBase(void)
{
  ...
  CT4 = 0;    // Mode Timer
	RCAP4 = -((SYSCLK/12)/100*11.1); // modifié
  T4 = RCAP4;
  TR4 = 1;                           // Timer4 démarré
  PT4 = 1;							  // Priorité Timer4 Haute
...
void Modif_Cfg_Globale (void)
{
    ...
    XBR2      = 0x58; // Configuration Hors du cadre TP1-BSE
		...
		P0MDOUT &= ~(1<<4);  //  P0.4 mis à 0 pour configurer l'entrer en open drain
		P0MDOUT &= ~(1<<5);  //  P0.5 mis à 0
		P0MDOUT &= ~(1<<6);  //  P0.6 mis à 0
		P0MDOUT &= ~(1<<7);  //  P0.7 mis à 0
	
		P1MDOUT |= (1<<0);  //  P1.0 mis à 1
		P1MDOUT |= (1<<1);  //  P1.1 mis à 1
