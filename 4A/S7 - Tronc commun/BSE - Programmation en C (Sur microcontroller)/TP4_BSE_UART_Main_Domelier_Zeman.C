...
void Cfg_CLK_UART0 (void){
	TCON |= (1<<6); // stop Timer 1
	TCON &= ~(1<<7); // flag timer 1 efface
	
	CKCON |= (1<<4); // use of the system clk
	TMOD |= (1<<5); // mode 8 bits autO reload
	TMOD &= ~(1<<7); // mode 8 bits autO reload
	TMOD &= ~(1<<6); // mode 8 bits autO reload
	TMOD &= ~(1<<4); // mode 8 bits autO reload
	...
	TH1 |= (1<<3);	// calcule
	// TL1
	TCON |= (1<<3); //devalidation de l'nterruption timer 1
	TCON |= (1<<6); // demarrage de T1
}
	
	

void Cfg_UART0 (void){
	SBUF0 |= (1<<0); // SBUF0.0 set à 1.
	
	SCON0 &= ~(1<<7);
	SCON0 |= (1<<6); // mode 1 : SM10 à 1 et SM00 à 0. Avec SSTAT0 qui reste à 0.
	SM20 = 1; 
	REN0 = 1;
	TI0 = 1;
	RI0 = 0;
	
	PCON |= (1<<7); // SMOd0 set à 1.	
	PCON &= ~(1<<6); //SMod1 clear a 0
